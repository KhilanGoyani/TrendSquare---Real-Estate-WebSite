// src/components/SpaciousHomes.js
import React, { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import { toast } from "react-toastify";
import "../CSS/CozyApartments.css"; 
import Navbar from "./Navbar";
import Footer from "./Footer";

export default function SpaciousHomes() {
    ...
    const fetchRentListings = async () => {
      try {
        ...
        const response = await fetch(
          "http://127.0.0.1:8000/api/users/rent_listings/?min_bhk=3&max_bhk=5"
        );
          ...
          {filteredListings.length > 0 ? (
            filteredListings.map((prop) => (
              <div key={prop.id} className="property-card">
                ...
                <p>
                  <strong>Rent:</strong> ₹{prop.rent_price.toLocaleString()}
                </p>
                <p>
                  <strong>BHK:</strong> {prop.bhk}
                </p>
                <p>
                  <strong>Area:</strong> {prop.area} sqft
                </p>
                <Link
                  to={`/rental-details/${prop.id}`}
